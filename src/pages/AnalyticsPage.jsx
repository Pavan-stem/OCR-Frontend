...
const AnalyticsPage = ({ filterProps }) => {
  ...
  // SSE Real-time Updates (Analytics Specific)
  useEffect(() => {
    const token = localStorage.getItem('token');
    if (!token) return;
    ...
    const connectSSE = () => {
      try {
        ...
        eventSource.onmessage = (event) => {
          if (event.data === 'refresh' || event.data === 'full') {
            console.log("Real-time refresh signal received (Analytics)");
            debouncedRefresh();
        ...
        eventSource.onerror = (err) => {
          console.error("✗ SSE Error (Analytics):", err);
        ...
        console.error("Failed to create EventSource (Analytics):", err);
      }
    };
  ...
  // Global Tree Refresh SSE (Sync with User Dashboard)
  useEffect(() => {
    const token = localStorage.getItem('token');
    if (!token) return;
    ...
    const initGlobalSSE = async () => {
      try {
        // 1. Get connection ID from global broadcaster
        const res = await fetch(`${API_BASE}/api/users/init-sse`, {
          method: "POST",
        ...
        // 2. Open global SSE stream
        globalEventSource = new EventSource(`${API_BASE}/api/sse/stream/${data.connection_id}`);

        globalEventSource.onmessage = (event) => {
          try {
            const message = JSON.parse(event.data);
            if (message.type === 'tree_refresh') {
              console.log('📡 Received global tree_refresh:', message.reason);
              debouncedRefresh();
            }
          } catch (e) {
            // Not a JSON message or different type
          }
        };
        ...
        console.error("Global SSE Init Error:", err);
      }
    };
