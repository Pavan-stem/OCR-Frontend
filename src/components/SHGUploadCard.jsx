...
const SHGUploadCard = ({
    ...
    const isReadyToSubmit = filesData.page1?.validated && filesData.page2?.validated;
                    ...
                    <div className="space-y-3">
                        ...
                        {!fileData.validated && (
                            <div className="flex items-center gap-2 text-[12px] text-red-600 font-bold px-1">
                                ...
                                <span>అప్లోడ్ చేసే ముందు పత్రం ధృవీకరించండి</span>
                            </div>
                        )}

                        {/* Redesigned Actions (Removal of individual upload, fixed overlaps) */}
                        <div className="flex flex-col gap-2 mt-1">
                            {/* Validate Button (ధృవీకరించండి) - Now spanning full width for text clarity */}
                            <button
                                onClick={() => onValidateFile(shg.shgId, pageKey === 'page1' ? 1 : 2)}
                                disabled={fileData.validated}
                                className={`flex items-center justify-center gap-2 py-2.5 rounded-lg font-bold text-[12px] transition-all shadow-sm ${
                                    fileData.validated 
                            ...
                            >
                                <CheckCircle size={16} /> 
                                <span>{fileData.validated ? 'ధృవీకరించబడింది' : 'ధృవీకరించండి'}</span>
                            </button>
                            ...
                            <div className="grid grid-cols-2 gap-2">
                                ...
                                <button
                                    ...
                                    className="flex items-center justify-center gap-2 py-2.5 bg-[#a55dfa] hover:bg-[#9245e6] text-white rounded-lg font-bold text-[12px] transition-all shadow-sm active:scale-95"
                                >
                                    <Eye size={16} /> 
    ...
    return (
        <div className={`relative bg-white rounded-xl shadow-md border transition-all ${
            rejectionInfo ? 'border-red-300' :
                        ...
                        {(hasFiles || isReadyToSubmit) && (
                            <div className="pt-4 border-t border-gray-100 mt-4">
                                <button
                                    ...
                                    className={`w-full py-3 rounded-xl font-bold text-sm flex items-center justify-center gap-3 transition-all shadow-md group ${
                                        isReadyToSubmit && !isUploading 
                                        ? 'bg-blue-600 hover:bg-blue-700 text-white transform active:scale-95' 
                                        : 'bg-gray-200 text-gray-500 cursor-not-allowed'
                                    }`}
                                >
                                    <Upload size={20} className={isReadyToSubmit ? 'animate-bounce group-hover:animate-none' : ''} />
                                    <span>{isUploading ? 'అప్లోడ్ అవుతోంది...' : 'పత్రం అప్లోడ్ చేయండి'}</span>
                                </button>
                                {!isReadyToSubmit && hasFiles && (
                                    <div className="flex items-center justify-center gap-1.5 mt-3 text-red-500">
                                        <AlertTriangle size={14} />
                                        <p className="text-[11px] font-bold">రెండు పత్రాలను ధృవీకరించిన తర్వాతే అప్లోడ్ చేయగలరు.</p>
                                    </div>
                                )}
                            </div>
                        )}
